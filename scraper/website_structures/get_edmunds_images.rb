...
class GetEdmundsImages
  ...
  def self.create_csv_file(writer, index, directory, params, image_elements)
    ...
    image_elements.each do |image|
      sleep 2
      image_link = convert_to_higher_resolution(get_image_link(image))
      puts 'Start:'
  ...
  def self.convert_to_higher_resolution(link)
    link.gsub(/150\.jpg/, '600.jpg')
  end

  def self.get_image_link(photo)
    photo.attr('src')
  end
  ...
  def self.create_image_directory(model_year, params)
    FileUtils::mkdir_p "edmunds_files/#{params[:manufacturer_name]}/#{params[:model_name]}/#{model_year}"
  end
